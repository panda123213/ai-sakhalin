...
while True:
    ...
    if 'отсортируй числа'in user_input.lower():
        with open('stage1_algorithms/numbers.txt','r',encoding='utf-8') as f:
            numbers=[int(x) for x in f.read().split() if x.lstrip('-').isdigit()]
        sorted_numbers=quick_sort(numbers)
        print(f'бот:стало:{sorted_numbers},было:{numbers}')
        continue
